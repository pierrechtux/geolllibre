...
2013_06_27__11_09_14: je dérive ça pour les analyses de type BLEG de Veritas.
"""
...
dbhost  = 'duran'
# user    = 'smiexplo'
# passw   = 'justement'    # curieusement, psql demande le mot de passe à Ity
# user    = 'tanguy'
user    = 'postgres'   # en dernier recours, on fait en tant que postgres.
passw   = 'flotouo'    # pas beau; ne pas le faire
...
key_values = []
for i in range(11):            #11 lignes d'en-tête au lieu de 12 pour les fichiers de FA
    line = file_in_reader.next()
    ch = 'SEPSEPSEPSEP'.join(line)
    line = (''.join(e for e in ch if (e.isalnum() or e == ' ' or e == '_' or e == '-' or e == '#'))).split('SEPSEPSEPSEP')
    #(k, v) = line[0:2]  #il y a parfois plus de 2 colonnes
    k = line[0]
    v = ' '.join(line[1:])
...
for (k, v) in key_values:
    ...
    if  k == 'Batch_No':
        jobno   = v
    #elif k == 'PO NUMBER':   # pas de PO NUMBER / orderno dans les fichiers de BLEG (BL002S)
    #    orderno = v

 # Vérifions si ça s'est bien passé, si on a bien un jobno et PAS un orderno:/*{{{*/ 
try:
...
for line in file_in_reader:
    sql_string += "(" + str(opid) + ", '" + str(labname) + "', '" + str(jobno) + "', '" 
    # + str(orderno) + "', '"
    sql_string += str(datasource) + "', '" + str(scheme) + "', '" + str(analyte) + "', '" + str(line[0]) + "', '" + str(line[1])  + "', '" + str(unit) + "'),\n"
...
sql_string = "UPDATE public.dh_sampling_grades SET au2_ppm = tmp.au_cy_avg_ppm FROM (SELECT opid, sample_id, avg(value_num) / 1000 AS au_cy_avg_ppm FROM lab_ana_results WHERE opid = 18 AND analyte = 'AU' AND scheme IN ('BL002S', 'CY001') AND unit = 'PPB' GROUP BY opid, sample_id) AS tmp WHERE (dh_sampling_grades.opid = tmp.opid AND dh_sampling_grades.sample_id = tmp.sample_id AND au2_ppm IS NULL);"
print(sql_string)
cur.execute(sql_string)
