...
--------------------+------------------------------------------------------------+-------+--------------
 ...
 public             | shift_reports                                              | table | data_admin	<= RENOMMÉE en dh_, ATTENTION!
...
CREATE TABLE occurrences (
    ...
    datasource integer,
    comments character varying,
    numauto_auto serial PRIMARY KEY,
    CONSTRAINT chk_status CHECK (((status)::text = ANY (ARRAY[('OCCUR'::character varying)::text, ('OREB'::character varying)::text, ('MINE'::character varying)::text, ('MINED'::character varying)::text, ('MCO'::character varying)::text, ('DISTRICT'::character varying)::text]))),
...
CREATE TABLE licences (
    ...
    numauto serial PRIMARY KEY,
    datasource integer,
    geometry_literal_description_plain_txt character varying,
    geometry_wkt character varying
);
COMMENT ON TABLE licences                                  IS 'Licences, tenements';
...
ALTER FUNCTION public.generate_cross_sections_array() OWNER TO postgres; -- tiens, et pourquoi donc??
