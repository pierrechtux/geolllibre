...
; routines appel�es par les programmes rebol de geolllibre


; Récup�ration des pr�f�rences (dbname dbhost user passw opid tmp_schema):
catch [
...
connection_db: does [ ;{{{ } } }
	; on fait une connexion � la base de donn�es:
...
run_query: func ["Utility function: sends a SQL query, returns the result as a block named sql_result; sql_result_fields contains the fields" sql] [ ; {{{ } } }
	...
	sql_result: copy db
	; TODO prendre en compte le r�sultat de requ�tes de type UPDATE ou DELETE
...
; des fonctions:
do_psql: func ["Prend du SQL en entr�e, et fait tourner psql avec, en renvoyant la sortie" sql_text] [ ;{{{ } } }
	;TODO: ajouter un raffinement /unaligned qui rajoute le flag "-A" pour psql
	;TODO: pouvoir choisir psql (pour les plateformes � la noix qui l'ont pas dans le $PATH...)
...
get_new_datasource_id: does [ ;{{{ } } }
	...
	; r�cup�re le premier datasource_id libre
...
get_datasource_dependant_information: func [ ;{{{ } } }
	...
	; �t des tables inutiles, o� datasource n'est pas r�f�renc�:
	;length? tables
	; == 43
	tables_inutiles: [
		["lex_datasource"]
	...
	; v�rifions en premier lieu que le datasource demand� existe bien dans lex_datasource:
	...
	if ((length? lex_datasource_record) = 0) [
		print rejoin ["-- Attention! datasource " datasource " introuvable dans public.lex_datasource pour l'op�ration " opid ", pas d'arr�t n�anmoins."]
		;return none 	; on laisse aller, pour pouvoir traiter des cas improbables o� des datasources "fant�mes" se prom�nent
	]
	output: make block! []
	foreach t tables [
		t: to-string t
		either error? try [
			...
			either ((to-integer nb_records_du_datasource) = 0) [
				;print rejoin ["-- Table " t ": datasource " datasource " non mentionn� pour op�ration " opid]
			] [
...
;>> get_datasource_dependant_information 912
;-- Table ancient_workings: datasource 912 non mentionn� pour op�ration 18
;-- Table baselines: datasource 912 non mentionn� pour op�ration 18
;-- Table dh_core_boxes: datasource 912 non mentionn� pour op�ration 18
;-- Table dh_density: datasource 912 non mentionn� pour op�ration 18
;-- Table dh_devia: datasource 912 non mentionn� pour op�ration 18
;-- Table dh_litho: datasource 912 non mentionn� pour op�ration 18
;-- Table dh_quicklog: datasource 912 non mentionn� pour op�ration 18
;-- Table dh_sampling_bottle_roll: datasource 912 non mentionn� pour op�ration 18
;-- Table dh_sampling_grades: datasource 912 non mentionn� pour op�ration 18
;-- Table dh_struct_measures: datasource 912 non mentionn� pour op�ration 18
;-- Table dh_tech: datasource 912 non mentionn� pour op�ration 18
;-- Table dh_thinsections: datasource 912 non mentionn� pour op�ration 18
;-- Table field_observations: datasource 912 non mentionn� pour op�ration 18
;-- Table field_observations_struct_measures: datasource 912 non mentionn� pour op�ration 18
;-- Table field_photos: datasource 912 non mentionn� pour op�ration 18
;-- Table geoch_ana: datasource 912 non mentionn� pour op�ration 18
;-- Table geoch_sampling: datasource 912 non mentionn� pour op�ration 18
;-- Table geoch_sampling_grades: datasource 912 non mentionn� pour op�ration 18
;-- Table gpy_mag_ground: datasource 912 non mentionn� pour op�ration 18
;-- Table grade_ctrl: datasource 912 non mentionn� pour op�ration 18
;-- Table index_geo_documentation: datasource 912 non mentionn� pour op�ration 18
;-- Table lab_ana_batches_reception: datasource 912 non mentionn� pour op�ration 18
;-- Table lab_ana_qaqc_results: datasource 912 non mentionn� pour op�ration 18
;-- **Table lab_ana_results contient 207.0 enregistrements correspondant au datasource 912 pour l'op�ration 18
;-- Table lex_codes: datasource 912 non mentionn� pour op�ration 18
;-- Table lex_standard: datasource 912 non mentionn� pour op�ration 18
;-- Table licences: datasource 912 non mentionn� pour op�ration 18
;-- Table mag_declination: datasource 912 non mentionn� pour op�ration 18
;-- Table occurrences: datasource 912 non mentionn� pour op�ration 18
;-- Table qc_sampling: datasource 912 non mentionn� pour op�ration 18
;-- Table qc_standards: datasource 912 non mentionn� pour op�ration 18
;-- Table shift_reports: datasource 912 non mentionn� pour op�ration 18
;-- Table surface_samples_grades: datasource 912 non mentionn� pour op�ration 18
;-- Table topo_points: datasource 912 non mentionn� pour op�ration 18
;== [["lab_ana_results" 207.0]]
;}}}
delete_datasource_and_dependant_information: func ["Delete a datasource from all tables from bdexplo where its datasource_id is mentioned" datasource] [ ; {{{ } } }
	...
	foreach i tables_a_traiter [
		...
		;prin rejoin ["-- D�TRUIRE LES " i/2 "ENREGISTREMENTS DANS " i/1 " (yYoO/nN)? "]
		;r: input
		;either any [(r = 'y') (r = 'Y') (r = 'o') (r = 'O')]  [
			...
			print sql
			;print "--on est timide pour le moment, on ne fait qu'afficher le SQL qui fera le boulot: � vous de le coller o� il convient"
		;] [
		;print " Records kept"
		;]
	]
	; �liminons de lex_datasource, bien s�r:
